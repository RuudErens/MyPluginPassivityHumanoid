...
void MyPluginPassivityHumanoid::init(mc_control::MCGlobalController & controller, const mc_rtc::Configuration & config)
{
  ...
  if(!robot.hasDevice<mc_rbdyn::VirtualTorqueSensor>("ExtTorquesVirtSensor"))
  {
    mc_rtc::log::error_and_throw<std::runtime_error>(
        "[MyPluginPassivityHumanoid][Init] No \"VirtualTorqueSensor\" with the name \"ExtTorquesVirtSensor\" found in "
  ...
  ctl.controller().datastore().make_call("PassivityPlugin::activated",
                                         [this]()
                                         {
                                           if(is_active_ == false) this->is_active_ = true;
                                         });
...
{
  // Get the robot that is controlled
  auto & ctl = static_cast<mc_control::MCGlobalController &>(controller);
  auto & robot = ctl.robot();

  // Check if Coriolis activation
  auto coriolis_activation = ctl.controller().datastore().get<std::string>("Coriolis");
  if(coriolis_activation.compare("Yes") == 0)
  {
    coriolis_indicator_ = true;
    coriolis_indicator_value_ = 1.0;
  }
  else
  {
    coriolis_indicator_ = false;
    coriolis_indicator_value_ = 0.0;
  }

  // If no encoder velocities, we do not compute anything
  if(robot.encoderVelocities().empty())
  {
    return;
  }

  // Get the joint torques computed by the QP solver
  rbd::paramToVector(robot.mbc().jointTorque, tau_Out_);
  tau_qp_ = tau_Out_ - tau_;

  // Fetch q_dot and q_ddot
  ...
  // Integration to have the velocity reference to calculate the error s
  alpha_r_ += alpha_d * dt_;

  alpha_ = alpha;
  alpha_d_ = alpha_d;

  // Complete the forward kinematics and velocity to compute the Coriolis matrix??
  ...
  for(int i = 0; i < motor_current_.size(); ++i)
  {
    ...
    if(std::isnan(motor_current_(i)))
    {
      tau_current_(i) = 0.0;
    }
    else
    {
      tau_current_(i) = motor_current_(i);
    }
  ...
  if(ctrl_mode.compare("Torque") == 0)
  {
    ctl.controller().datastore().call("PassivityPlugin::activated");
  }
  ...
  if(is_active_)
  {
    ...
    // Exponential integration for perc_ to maintain continuity in case the torque is constrained by the QP-Based
    // Anti-Windup
  ...
  else
  {
    K_ = lambda_massmatrix_ * fd_->H() + lambda_id_ * Eigen::MatrixXd::Identity(robot.mb().nrDof(), robot.mb().nrDof());
    slow_filtered_s_ =
        (1 - fast_filter_weight_) * (K_ + coriolis_indicator_value_ * C_).inverse() * (tau_current_ - tau_qp_);
    s_ = slow_filtered_s_;
...
void MyPluginPassivityHumanoid::addGUI(mc_control::MCGlobalController & controller)
{
  ...
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
                  mc_rtc::gui::Button("Activate Plugin", [this]()
                                      { is_active_ = true, mc_rtc::log::info("IntegralFeedback activated"); }));
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
  ...
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
                  mc_rtc::gui::NumberInput(
                      "Anti-Windup percentage", [this]() { return this->perc_; },
  ...
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
                  mc_rtc::gui::NumberInput(
                      "Gain Mass matrix", [this]() { return this->lambda_massmatrix_; },
  ...
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
                  mc_rtc::gui::NumberInput(
                      "Gain Identity matrix", [this]() { return this->lambda_id_; },
  ...
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
                  mc_rtc::gui::NumberInput(
                      "Slow filter phi", [this]() { return phi_slow_; },
                      [this](double phi)
                      {
                        phi_slow_ = phi;
  ...
  gui->addElement({"Plugins", "Integral term feedback", "Configure"},
                  mc_rtc::gui::NumberInput(
                      "Fast filter phi", [this]() { return phi_fast_; },
                      [this](double phi)
                      {
                        phi_fast_ = phi;
  ...
  // Logs
  gui->addElement(
      {"Plugins", "Integral term feedback", "Logs"},
      mc_rtc::gui::ArrayLabel("Ref velocity", ctl.controller().robot().refJointOrder(), [this]() { return alpha_r_; }));

  gui->addElement({"Plugins", "Integral term feedback", "Logs"},
                  mc_rtc::gui::ArrayLabel("New non filtered integral term", ctl.controller().robot().refJointOrder(),
                                          [this]() { return new_s_; }));

  gui->addElement({"Plugins", "Integral term feedback", "Logs"},
                  mc_rtc::gui::ArrayLabel("Past non filtered integral term", ctl.controller().robot().refJointOrder(),
                                          [this]() { return prev_s_; }));

  gui->addElement({"Plugins", "Integral term feedback", "Logs"},
                  mc_rtc::gui::ArrayLabel("Slow filtered integral term", ctl.controller().robot().refJointOrder(),
                                          [this]() { return slow_filtered_s_; }));

  gui->addElement({"Plugins", "Integral term feedback", "Logs"},
                  mc_rtc::gui::ArrayLabel("Fast filtered integral term", ctl.controller().robot().refJointOrder(),
                                          [this]() { return fast_filtered_s_; }));

  gui->addElement(
      {"Plugins", "Integral term feedback", "Logs"},
      mc_rtc::gui::ArrayLabel("Integral term", ctl.controller().robot().refJointOrder(), [this]() { return s_; }));

  gui->addElement(
      {"Plugins", "Integral term feedback", "Logs"},
      mc_rtc::gui::ArrayLabel("Torque sent", ctl.controller().robot().refJointOrder(), [this]() { return tau_; }));

  gui->addElement(
      {"Plugins", "Integral term feedback", "Logs"},
      mc_rtc::gui::ArrayLabel("Torque QP", ctl.controller().robot().refJointOrder(), [this]() { return tau_qp_; }));

  // Plots
  gui->addElement(
      {"Plugins", "Integral term feedback", "Plots"}, mc_rtc::gui::ElementsStacking::Horizontal,
      mc_rtc::gui::Button("Plot integral term",
                          [this, gui]()
                          {
                            gui->addPlot(
                                "Integral term feedback",
                                mc_rtc::gui::plot::X("t", [this]() { return static_cast<double>(count_) * dt_; }),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(0)", [this]() { return s_[0]; }, mc_rtc::gui::Color::Blue),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(1)", [this]() { return s_[1]; }, mc_rtc::gui::Color::Red),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(2)", [this]() { return s_[2]; }, mc_rtc::gui::Color::Green),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(3)", [this]() { return s_[3]; }, mc_rtc::gui::Color::Yellow),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(4)", [this]() { return s_[4]; }, mc_rtc::gui::Color::Magenta),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(5)", [this]() { return s_[5]; }, mc_rtc::gui::Color::Cyan),
                                mc_rtc::gui::plot::Y(
                                    "Integral term q(6)", [this]() { return s_[6]; }, mc_rtc::gui::Color::Black));
                          }),
...
{
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_active",
                                               [&, this]() { return this->is_active_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_changing",
                                               [&, this]() { return this->is_changing_; });

  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_type",
                                               [&, this]() { return static_cast<int>(this->filtered_activated_); });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_percentage_of_bounds",
                                               [&, this]() { return this->perc_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_coriolis_indicator_value",
                                               [&, this]() { return this->coriolis_indicator_value_; });

  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_gain_mass_matrix",
                                               [&, this]() { return this->lambda_massmatrix_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_gain_identity",
                                               [&, this]() { return this->lambda_id_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_slow_phi",
                                               [&, this]() { return this->phi_slow_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_fast_phi",
                                               [&, this]() { return this->phi_fast_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_slow_exp_phi",
                                               [&, this]() { return this->exp_phi_slow_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_fast_exp_phi",
                                               [&, this]() { return this->exp_phi_fast_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_ratio",
                                               [&, this]() { return this->fast_filter_weight_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_integral_of_reference_acceleration",
                                               [&, this]() { return this->alpha_r_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_velocity",
                                               [&, this]() { return this->alpha_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_acceleration",
                                               [&, this]() { return this->alpha_d_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_K", [&, this]()
                                               { return static_cast<Eigen::VectorXd>(this->K_.diagonal()); });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_s", [&, this]() { return this->s_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_new_s",
                                               [&, this]() { return this->new_s_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_previous_s",
                                               [&, this]() { return this->prev_s_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_slow_s",
                                               [&, this]() { return this->slow_filtered_s_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_filter_fast_s",
                                               [&, this]() { return this->fast_filtered_s_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_passivity",
                                               [&, this]() { return this->tau_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_qp",
                                               [&, this]() { return this->tau_qp_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_out",
                                               [&, this]() { return this->tau_Out_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_coriolis",
                                               [&, this]() { return this->tau_coriolis_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_current",
                                               [&, this]() { return this->tau_current_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_sum",
                                               [&, this]() { return this->tau_sum_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_Ks",
                                               [&, this]() { return this->tau_K_s_; });
  controller.controller().logger().addLogEntry("MyPluginPassivityHumanoid_torque_after_solve",
                                               [&, this]() { return this->tau_after_solve_; });
...
void MyPluginPassivityHumanoid::torque_continuity(double lambda_massmatrix, double lambda_id)
{
  ...
  Eigen::MatrixXd L_new = coriolis_indicator_value_ * C_ + lambda_massmatrix * fd_->H()
                          + lambda_id * Eigen::MatrixXd::Identity(s_.size(), s_.size());
